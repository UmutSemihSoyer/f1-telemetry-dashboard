"""
replay.py — F1 2022 Session Replay
Replays a past session from SQLite or a CSV file into live_data.json.
Usage: python replay.py [--session SESSION_ID] [--speed 2.0] [--csv PATH]
"""
import sqlite3
import json
import time
...
def replay_session(df: pd.DataFrame, speed: float = 1.0):
    """
    Replay lap rows to live_data.json, simulating real-time pace.
    Each lap row is written as a snapshot with a delay proportional to lap time.
    """
    print(f"[REPLAY] {len(df)} laps at speed x{speed}")
    ...
    for i, (_, row) in enumerate(df.iterrows(), 1):
        ...
        print(f"  Lap {int(row.get('lap_num', i)):3d} | {lap_t_s:.3f}s | "
...
if __name__ == "__main__":
    ...
    if args.csv:
        ...
        if not sessions:
            print("[REPLAY] No sessions in database. Run 'python run_all.py' first.")
        else:
            print("\nAvailable Sessions:")
            print(f"{'ID':<25} {'Laps':>6} {'Best (ms)':>14}")
            print("-" * 50)
            for sid, laps, best in sessions:
                print(f"{sid:<25} {laps:>6} {best:>14}")

            target = args.session or sessions[0][0]
            print(f"\n[REPLAY] Playing session: {target} (x{args.speed})")
            df = load_session(target)
            if df.empty:
                print(f"[REPLAY] Session not found: {target}")
            else:
                replay_session(df, speed=args.speed)
